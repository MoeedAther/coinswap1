...
import {set_selcur, set_getcur, set_amountcur, set_inputcur, setsendindex, setgetindex} from "./features/offerSlice.js";
...
function ExchangeCrypto(props) {
  ...
  let cur = filtered.length > 0 ? filtered : cr;
  let cur1 = filtered1.length > 0 ? filtered1 : cr;
  ...
  var fetchMultipleData = async (sel, get, amount) => {
   try{    
    setload(true);
    // const url=URL+"pricecheck";
    const url="http://localhost:5002/pricecheck";
    const options={
      method:"POST",
      headers:{
    ...
    console.log(data)
     if(amount>data.from.min){
      setAmountErr(false);
      setgval(data.from.rate)
      setload(false);
      console.log(data.from.min)
     }else{
      setAmountErr(true);
      setAmountMin(data.from.min);
      setload(true);
     }
  }catch{
    setload(true);
  ...
  const handleExchange = async (e) => {
    fetchMultipleData(cr[exchangesenddata]?.symbol, cr[exchangegetdata]?.symbol,e);
  };
  ...
  //...........................useEffects.............................//
    ...
    useEffect(() => {
    dispatch(set_amountcur(0.1))
    getCryptodata();
    dispatch(setsendindex(0))
    dispatch(setgetindex(1))

    localStorage.setItem("sendindex",0)
    localStorage.setItem("getindex",1)

    dispatch(set_selcur(initval1))
    dispatch(set_getcur(initval2))

    localStorage.setItem("selcur", initval1 )
    localStorage.setItem("getcur", initval2 )

    document.addEventListener("click", handleClickOutside, true);
    console.log(initval1,initval2,sval)
  ...
  return (
    <div className="formContainer formContainer2" style={{ height: props.boxHeight }}>
      <div className="tabs">
        <span
          onClick={() => {
            setexc(true);
            ...
            {!showexchangesenddrop ? (
              <>
                <input
                  ...
                  min={0.1}
                  onChange={(e) => {
                    replaceFirstCommaWithPeriodAndRemoveAlphabets(e.target.value);
                    handleExchange(e.target.value);
                  
                    dispatch(set_inputcur(sval));  
                    // const inputValue = e.target.value;
                    // const sanitizedValue = inputValue.replace(/[^0-9.]/g, '') // Allow only digits and a single dot
                    // .replace(/(\..*)\./, '$1'); // Remove extra dots
              
                  // if (sanitizedValue === '' || (sanitizedValue === '.' || parseFloat(sanitizedValue) <= 9)) {
                  //   // Check if the input is empty, a single dot, or the value is less than or equal to 9
                  //   console.log("string formatting")
                  //   setsval(sanitizedValue);
                  //   handleExchange(sanitizedValue);
                  //   dispatch(set_inputcur(sanitizedValue));
                  // }
          ...
          <div className="row" style={{ height: "50px" }}>
            ...
            <div className={props.switchBtn}>
              <button
                ...
                onClick={()=>{
                  var sendindex = null;
                  var getindex = exchangegetdata;
                  var exchangevar =null;

                  //Swap variables
                  exchangevar = exchangesenddata;
                  sendindex=getindex;
                  getindex=exchangevar;

                  setexchangesenddata(exchangegetdata);
                  setexchangegetdata(exchangevar);

                  //Setting send index for viewoffer page
                  dispatch(setsendindex(sendindex))
                  dispatch(setgetindex(getindex))

                  localStorage.setItem("sendindex",sendindex)
                  localStorage.setItem("getindex",getindex)
                  ...
                  console.log("send index", sendindex);
                ...
                <div className="srch_grp">
                    <ul>
                      <li 
                        ...
                        onClick={() => {scrollToPopular();handleItemClick2(0); setShowPopular2(true)}}
                      >
                        POPULAR CURRENCIES
                      </li>
                      <li 
                        ...
                        onClick={() => {scrollToPopular2();handleItemClick2(1);setShowPopular2(false)}}
                      >
                        OTHER CURRENCIES
                      </li>
                    </ul>
...
>
  ...
  >
    ...
    {/* {x?.symbol.toUpperCase()} */}
      ...
      <div 
        className="nav-margin nav-color"
        style={{ display: props.display, marginRight:'35px' }}
      >
        <button className="offerbtn" style={{width:'100%', backgroundColor: amountErr ? "#8f8484" : '', boxShadow: amountErr ? '0 4px 30px rgba(0, 0, 0, .2), 0 4px 50px rgb(164 159 148 / 50%), 0 60px 150px rgb(162 158 150 / 62%)' : ''}} onClick={handleNavigate} disabled={amountErr}><span>{lang.viewoffers}</span></button>
      </div>
